...
echo "#"
echo "# save path /tmp/Aqara_G3_nodered_2device.json"
echo "# second device"
echo "#"
...
echo "# Insert your country for timeformat example: es-ES/en-UK/de-DE/it-IT/pt-PT/es-ES"
...
echo "rm -rf /tmp/*Aqara_G3_nodered.json* && wget -q https://raw.githubusercontent.com/sdavides/AqaraPOST-Homeassistant/main/Aqara_G3_nodered_2device.json --output-document=/tmp/tmp_Aqara_G3_nodered.json" >> /tmp/Aqara_G3_nodered.sh
echo "echo download complete" >> /tmp/Aqara_G3_nodered.sh
echo "echo insert value" >> /tmp/Aqara_G3_nodered.sh
echo "cat /tmp/tmp_Aqara_G3_nodered.json | sed -e 's/XXXXXXTOKENXXXXXXXXXXX/$NEWTOKEN/g' | sed -e 's/XXXXXXAPPIDXXXXXXXXXXX/$NEWAPPID/g' |sed -e 's/lumi1.XXXXXXXXXXXX/$NEWLUMI1/g' | sed -e 's/it-IT/$TIMEZONE/g' | sed -e 's/rpc-ger.aqara.com/$aqara_url/g' > /tmp/Aqara_G3_nodered_2device.json" >> /tmp/Aqara_G3_nodered.sh
